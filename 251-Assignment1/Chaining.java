...
public class Chaining {
    ...
    public int insertKey(int key){
        ...
        if(Table.get(hashedKey).size()==0){
            ArrayList<Integer> arrayList= Table.get(hashedKey);
            arrayList.add(key);
            return 0;
        }
        else{
            ArrayList<Integer> existingHashArray = Table.get(hashedKey);
            int collisions = existingHashArray.size();
            existingHashArray.add(key);
            return collisions;
        }
    }
